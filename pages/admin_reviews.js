...
async function refreshTokenRequest(refreshToken){
  ...
  const endpoint = 'http://34.218.112.35/auth/token'
  const payload = {
      token: refreshToken
  }
  ...
  const func = async() => {
    ...
    const token2 = cookieCutter.get('refreshtoken')
       ...
       // API endpoint where we send form data.
        const endpoint = 'http://34.218.112.35/admin/reviews'
  ...
  return (
    <div className='bgimg3'>
        ...
        <Admin_reviews key={added_properties_list.properties.property_id} reviews={added_properties_list.properties} arr={key}/>
        {/* <Admin_reviews reviews={reported_reviews.properties}/> */}
